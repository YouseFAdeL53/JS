...
let getHeight = scrollHeight - clientHeight;


window.addEventListener("scroll", () => {
  let scrollTop = document.documentElement.scrollTop;
  // get Width from scrollTop
  myScroll.style.width = `${(scrollTop / getHeight) * 100}%`
})
...
let myBoxes = Array.from(document.querySelectorAll(".slider .box"));

// getLength

let boxLength = myBoxes.length;
...
createUl.setAttribute("id", "pagaintion-ul");

createUl.className = "pagaintion-ul";

document.querySelector(".slider").appendChild(createUl);

// loop for create li

for (var l = 1; l <= boxLength; l++) {

  let createLi = document.createElement("li");

  createLi.setAttribute("data-index", l);

  createUl.appendChild(createLi)

}

let newUl = document.getElementById("pagaintion-ul");

let newLi = Array.from(document.querySelectorAll("#pagaintion-ul li"));

for (var l = 0; l < newLi.length; l++) {

  newLi[l].onclick = function () {
...
function checker() {
  ...
  myBoxes[currentSlide - 1].classList.add("active");
...
function prevButton() {
  ...
  if (prev.classList.contains("disapled")) {
    return false;
  } else {
    currentSlide--;
    checker();
...
function nextButton() {
  ...
  if (next.classList.contains("disapled")) {
    return false;
  } else {
    currentSlide++;
    checker();
